...
import type { OnlishopMessageTypes } from './message-types';
...
/**
 * Resembles the options that are available on any OnlishopMessageType.
 */
 ...
 * @internal
 */
export type MessageDataType<TYPE extends keyof OnlishopMessageTypes> = Omit<OnlishopMessageTypes[TYPE], 'responseType'>;

/**
 * This is the structure of the data which will be send with {@link send}
 * @internal
 */
export type OnlishopMessageSendData<MESSAGE_TYPE extends keyof OnlishopMessageTypes> = {
  _type: MESSAGE_TYPE,
 ...
 * @internal
 */
export type OnlishopMessageResponseData<MESSAGE_TYPE extends keyof OnlishopMessageTypes> = {
  _type: MESSAGE_TYPE,
  _response: OnlishopMessageTypes[MESSAGE_TYPE]['responseType'] | null,
  _callbackId: string,
}
 ...
 */
export function send<MESSAGE_TYPE extends keyof OnlishopMessageTypes>(
  type: MESSAGE_TYPE,
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  _targetWindow?: Window,
  _origin?: string
): Promise<OnlishopMessageTypes[MESSAGE_TYPE]['responseType'] | null> {
  ...
  const messageData: OnlishopMessageSendData<MESSAGE_TYPE> = {
    _type: type,
    _data: sendData,
  ...
  let serializedData = serialize(messageData) as OnlishopMessageSendData<MESSAGE_TYPE>;
  ...
  if (_origin) {
    ...
    if (validationErrors) {
      ...
      if ([
        ...
        }) as OnlishopMessageSendData<MESSAGE_TYPE>;
      }
  ...
  return new Promise((resolve, reject) => {
    const callbackHandler = function(event: MessageEvent<string>):void {
      ...
      let onlishopResponseData;
      // Try to parse the json response
      try {
        onlishopResponseData = JSON.parse(event.data) as unknown;
      } catch {
      ...
      if (!isMessageResponseData<MESSAGE_TYPE>(onlishopResponseData)) {
        return;
      }

      // Only execute if response value exists
      if (!onlishopResponseData.hasOwnProperty('_response')) {
        return;
      }

      // Deserialize methods etc. so that they are callable in JS
      const deserializedResponseData = deserialize(onlishopResponseData, event) as OnlishopMessageResponseData<MESSAGE_TYPE>;
...
export type HandleMethod<MESSAGE_TYPE extends keyof OnlishopMessageTypes> = (
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  additionalInformation: { _event_: MessageEvent<string> }
) => Promise<OnlishopMessageTypes[MESSAGE_TYPE]['responseType']> | OnlishopMessageTypes[MESSAGE_TYPE]['responseType'];
 ...
 */
export function handle<MESSAGE_TYPE extends keyof OnlishopMessageTypes>
(
  type: MESSAGE_TYPE,
  ...
  : () => void
{
  const handleListener = async function(event: MessageEvent<string>): Promise<void> {
    ...
    let onlishopMessageData;

    // Try to parse the json file
    try {
      onlishopMessageData = JSON.parse(event.data) as unknown;
    } catch {
      // Fail silently when message is not a valid json file
      return;
    }
    ...
    // Check if messageData is valid
    if (!isMessageData<MESSAGE_TYPE>(onlishopMessageData)) {
      return;
    }

    // Deserialize methods etc. so that they are callable in JS
    const deserializedMessageData = deserialize(onlishopMessageData, event) as OnlishopMessageSendData<MESSAGE_TYPE>;
    ...
    const responseValue = await Promise.resolve((() => {
       ...
       */
      const validationErrors = validate({
        serializedData: onlishopMessageData,
        origin: event.origin,
        type: type,
    ...
    const responseMessage: OnlishopMessageResponseData<MESSAGE_TYPE> = {
    ...
    const serializedResponseMessage = ((): OnlishopMessageResponseData<MESSAGE_TYPE> => {
      let serializedMessage = serialize(responseMessage) as OnlishopMessageResponseData<MESSAGE_TYPE>;
      ...
      if (validationErrors) {
        ...
        serializedMessage = serialize(serializedMessage) as OnlishopMessageResponseData<MESSAGE_TYPE>;
      }
...
export function publish<MESSAGE_TYPE extends keyof OnlishopMessageTypes>(
  type: MESSAGE_TYPE,
  data: OnlishopMessageTypes[MESSAGE_TYPE]['responseType'],
  sources: {
    source: Window,
    origin: string,
    sdkVersion: string | undefined,
...
export function subscribe<MESSAGE_TYPE extends keyof OnlishopMessageTypes>(
  type: MESSAGE_TYPE,
  method: (data: OnlishopMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>
): () => void {
...
export function createSender<MESSAGE_TYPE extends keyof OnlishopMessageTypes>
(messageType: MESSAGE_TYPE, baseMessageOptions: MessageDataType<MESSAGE_TYPE>)
:(messageOptions?: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions) => Promise<OnlishopMessageTypes[MESSAGE_TYPE]['responseType']>

// SENDER WITH PARTIAL ARGUMENTS (ARGUMENTS DEFINED IN BASE OPTIONS ARE OMITTED)
export function createSender<MESSAGE_TYPE extends keyof OnlishopMessageTypes, BASE_OPTIONS extends Partial<MessageDataType<MESSAGE_TYPE>>>
(messageType: MESSAGE_TYPE, baseMessageOptions: BASE_OPTIONS)
:(messageOptions: Omit<MessageDataType<MESSAGE_TYPE>, keyof BASE_OPTIONS> & BaseMessageOptions) => Promise<OnlishopMessageTypes[MESSAGE_TYPE]['responseType']>
 ...
 */
export function createHandler<MESSAGE_TYPE extends keyof OnlishopMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
 ...
 * defined and can be hidden.
 */
export function createSubscriber<MESSAGE_TYPE extends keyof OnlishopMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
  return (method: (data: OnlishopMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>, id?: string) => {
    if (!id) {
      return subscribe(messageType, method);
    }

    const wrapper = (data: OnlishopMessageTypes[MESSAGE_TYPE]['responseType']): void => {
      if (data.id === id) {
        void method(data);
      }
    };
 ...
 */
function isMessageData<MESSAGE_TYPE extends keyof OnlishopMessageTypes>(eventData: unknown): eventData is OnlishopMessageSendData<MESSAGE_TYPE> {
  const onlishopMessageData = eventData as OnlishopMessageSendData<MESSAGE_TYPE>;

  return !!onlishopMessageData._type
         && !!onlishopMessageData._data
         && !!onlishopMessageData._callbackId;
}

// OnlishopMessageTypes[MESSAGE_TYPE]['responseType']
function isMessageResponseData<MESSAGE_TYPE extends keyof OnlishopMessageTypes>(eventData: unknown): eventData is OnlishopMessageResponseData<MESSAGE_TYPE> {
  const onlishopMessageData = eventData as OnlishopMessageResponseData<MESSAGE_TYPE>;

  return !!onlishopMessageData._type
         && !!onlishopMessageData.hasOwnProperty('_response')
         && !!onlishopMessageData._callbackId;
}
