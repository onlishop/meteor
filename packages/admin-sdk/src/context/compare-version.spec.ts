import getCompareIsOnlishopVersion from './compare-version';

type Comparator = '=' | '!=' | '<' | '>' | '<=' | '>=';

describe('getCompareOnlishopVersion', () => {
  const mock = (version: string) => () => Promise.resolve(version);

  describe('It works with onlishop versions and semver versions', () => {
    it.each<[boolean, string, Comparator, string]>([
      ...
      [false, '6.8.2', '=', '6.6.9.2'],
      [false, '6.8.2', '=', '6.9.2'],
    ])("returns %s for '%s %s %s'", async (exptectedResult, onlishopVersion, comparator, comparedVersion) => {
      const compareIsOnlishopVersion = getCompareIsOnlishopVersion(mock(onlishopVersion));

      expect(await compareIsOnlishopVersion(comparator, comparedVersion)).toBe(exptectedResult)
    });
  });
