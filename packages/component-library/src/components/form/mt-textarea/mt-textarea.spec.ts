...
describe("mt-textarea", () => {
  ...
  it("emits the changed value when the user moves focus to another element", async () => {
    ...
    // ACT
    await userEvent.type(screen.getByRole("textbox"), "Onlishop");
    await userEvent.tab();

    // ASSERT
    expect(handler).toHaveBeenNthCalledWith(1, "Onlishop");
  });
  ...
  it("does not allow typing more characters than the specified max length", async () => {
    ...
    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("Onli");
    expect(handler).toHaveBeenNthCalledWith(4, "Onli");
  });
  ...
  it("is possible to edit value of textarea with unlinked inheritance", async () => {
    ...
    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("Onlishop");
