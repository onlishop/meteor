 ...
 */
import type Repository from "@onlishop/meteor-admin-sdk/es/data/repository";
import Criteria from "@onlishop/meteor-admin-sdk/es/data/Criteria";
import { useRepository } from "@onlishop/meteor-admin-sdk/es/data/composables/useRepository";
import { getRepository } from "@onlishop/meteor-admin-sdk/es/data/composables/getRepository";
import MtDataTable, {
  type ColumnChanges,
...
const handleItemDelete = async ({ id }: { id: string }) => {
  ...
  if (isInIframe && !props.forceRealModal) {
    const { ui } = await import("@onlishop/meteor-admin-sdk");
