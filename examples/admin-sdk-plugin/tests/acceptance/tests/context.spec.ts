...
test("@sdk: subscribe on locale changes", async ({
  ...
  await ShopAdmin.page.locator("#sw-field--confirm-password").fill("onlishop");
...
test("@sdk: get current onlishop version", async ({ ShopAdmin }) => {
  await ShopAdmin.page.locator(".sw-order").click();
  ...
  await frame
    .locator("button", { hasText: "Get current Onlishop version" })
    .click();

  await expect(
    frame.locator("p", { hasText: "Onlishop version: 6." }),
  ).toBeVisible();
});
