<template>
  <SwCard title="Context">
    ...
    <div class="context-functionality-buttons">
      ...
      <SwButton @click="getCurrentOnlishopVersion">
...
import { context, notification } from '@onlishop/meteor-admin-sdk';
import { SwCard, SwButton } from '@onlishop/meteor-component-library';
...
async function getCurrentOnlishopVersion() {
  const currentOnlishopVersion = await context.getOnlishopVersion();

  notification.dispatch({
    title: 'Current Onlishop version',
    message: `
    Your current Onlishop version is "${currentOnlishopVersion}".
    `
  })
}
